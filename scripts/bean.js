...
class HealingBean extends Bean {
  constructor(x, y) {
    super(x, y, 10, 2, 1, beanURLs.MINT_2);
    this.t = 0;
  }
  update(dt) {
...
class FreezeBean extends Bean {
  static moveSpeed = 50;
  constructor(x, y) {
    super(x, y, 10, 2, 1, beanURLs.BLUE_2);
    this.t = 0;
  }
  update(dt) {
    this.t += dt * freezeMultiplier;
...
class DoubleBean extends Bean {
  static moveSpeed = 50;
  constructor(x, y) {
    super(x, y, 10, 2, 1, beanURLs.PINK_2);
    this.t = 0;
  }
  update(dt) {
    this.t += dt * freezeMultiplier;
...
class GlassBean extends Bean {
  constructor(x, y, speed = GlassBean.movingBeanSpeed) {
    super(x, y, 10, 2, 1, beanURLs.COLORLESS_2);
...
class ShieldBean extends Bean {
  constructor(x, y) {
    super(x, y, 10, 2, 1, beanURLs.GRAY_2);
    this.t = 0;
  }
  update(dt) {
    this.t += dt * freezeMultiplier;
  ...
  destroy(effect = true) {
    if (effect) {
      ...
      for (let bean of allBeans()) {
        if (bean instanceof DangerousThing && bean.blockedByGlass) bean.destroy();
      }
      Shield.start(8000);
    }
    super.destroy();
...
class SilverBean extends Bean {
  static speed = 280;
  constructor(x, y) {
    super(x, y, 0, 10, 1, beanURLs.GOLD_2);
...
class TaroBean extends Bean {
  constructor(x, y, speed = TaroBean.movingBeanSpeed) {
    super(x, y, 0, 1, 1, beanURLs.TARO_2);
    let angle = Math.random() * Math.PI * 2;
    this.velX = Math.cos(angle) * speed;
    this.velY = Math.sin(angle) * speed;
    this.t = 0;
...
class BlackBean extends Bean {
  constructor(x, y) {
    super(x, y, 25, 4, 15, beanURLs.WHITE_1);
